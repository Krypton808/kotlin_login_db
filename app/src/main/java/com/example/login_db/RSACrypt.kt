...
import com.example.login_db.Base64.encode
...
/*fun main(args: Array<String>) {
    ...
    //字符串转成秘钥对对象
    val kf = KeyFactory.getInstance("RSA")
    val privateKey = kf.generatePrivate(PKCS8EncodedKeySpec(decode(privateKeyStr)))
    val publicKey = kf.generatePublic(X509EncodedKeySpec(decode(publicKeyStr)))
...
/*fun main(){
    val input = "撒看见迪卡基地啊十九点"

    val encrypt = RSACrypt.encryptByPrivateKey(input, RSACrypt.getPrivateKey())
    val decryptByPublicKey = RSACrypt.decryptByPublicKey(encrypt, RSACrypt.getPublicKey())
    println("私钥加密 = $encrypt")
    println("公钥解密 =  $decryptByPublicKey")
